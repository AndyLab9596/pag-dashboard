...
import { useAdminViewEvaluationsQuery } from 'app/generated/graphql';
...
const Evaluations: React.FC<Props> = ({ filter, cycleId, rowAction, ...rest }) => {
  const toast = useToastStatus();
  const { filterVariables, setCurrentPage } = useFilters({ filter });
  const { sort, onSort } = useSortServerSide(undefined, 'actionSort');
  const { data, loading: queryLoading } = useAdminViewEvaluationsQuery({
    variables: {
      ...filterVariables,
      cycleId: cycleId,
      actionSort: {
  ...
  let totalData = data?.adminViewEvaluations.total;

  const users = useMemo(() => {
    if (!data) return [];

    return data?.adminViewEvaluations.data.map(item => ({
      ...item.user,
      ...item,
    }));
  }, [data]);
  ...
  return (
    <Table
      ...
      controlledPageIndex={data?.adminViewEvaluations.page ? data?.adminViewEvaluations.page - 1 : undefined}
    />
  );
};
